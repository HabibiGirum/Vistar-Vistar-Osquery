...
# Define the osquery executable path with double quotes to handle spaces
$osquery_executable = '"C:\Program Files\osquery\osqueryi.exe"'
...
while ($true) {
    ...
    foreach ($query in $osquery_queries.Keys) {
        try {
            # Use the Start-Process cmdlet to handle paths with spaces
            $osquery_output = Start-Process -FilePath $osquery_executable -ArgumentList "--json", $query -PassThru -Wait -NoNewWindow
            $osquery_data = $osquery_output.StandardOutput.ReadToEnd() | ConvertFrom-Json
